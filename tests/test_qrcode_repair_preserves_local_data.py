...
class _FakeMainWindow:
    ...
    def _abandon_closed_session(self, token):
        # See MainWindow._abandon_closed_session() — _close_active_session()
        # now calls this so a deliberately-closed session stops being the
        # single "unambiguous" candidate _recover_active_session_token()
        # would otherwise hand back on the next launch (issue #155).
